...
int main(void) {
  ...
  Model characterModel =
      LoadModel("resources/gltf/greenman.glb"); // Load character model
  Model equipModel[BONE_SOCKETS] = {
      LoadModel("resources/gltf/greenman_hat.glb"), // Index for the hat
                                                           // model is the same
                                                           // as BONE_SOCKET_HAT
      LoadModel(
          "resources/gltf/greenman_sword.glb"), // Index for the sword
                                                       // model is the same as
                                                       // BONE_SOCKET_HAND_R
      LoadModel(
          "resources/gltf/greenman_shield.glb") // Index for the shield
                                                       // model is the same as
                                                       // BONE_SOCKET_HAND_L
  };
  ...
  ModelAnimation *modelAnimations =
      LoadModelAnimations("resources/gltf/greenman.glb", &animsCount);
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    Quaternion characterRotate =
        QuaternionFromAxisAngle((Vector3){0.0f, 1.0f, 0.0f}, angle * DEG2RAD);
    characterModel.transform =
        MatrixMultiply(QuaternionToMatrix(characterRotate),
                       MatrixTranslate(position.x, position.y, position.z));
    UpdateModelAnimation(characterModel, anim, (float)animCurrentFrame);
    DrawMesh(characterModel.meshes[0], characterModel.materials[1],
             characterModel.transform);
    ...
    for (int i = 0; i < BONE_SOCKETS; i++) {
      ...
      Transform *transform =
          &anim.keyframePoses[animCurrentFrame][boneSocketIndex[i]];
      Quaternion inRotation =
          characterModel.skeleton.bindPose[boneSocketIndex[i]].rotation;
      Quaternion outRotation = transform->rotation;

      // Calculate socket rotation (angle between bone in initial pose and same
      // bone in current animation frame)
      Quaternion rotate =
      ...
      // Transform the socket using the transform of the character (angle and
      // translate)
      matrixTransform =
          MatrixMultiply(matrixTransform, characterModel.transform);

      // Draw mesh at socket position with socket angle rotation
      DrawMesh(equipModel[i].meshes[0], equipModel[i].materials[1],
               matrixTransform);
    }
    ...
    DrawText("Use the 1,2,3 to toggle shown of hat, sword and shield", 10, 60,
             20, GRAY);
