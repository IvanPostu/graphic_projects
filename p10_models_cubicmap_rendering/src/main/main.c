...
int main(void) {
  ...
  Image image =
      LoadImage("resources/cubicmap.png"); // Load cubicmap image (RAM)
  Texture2D cubicmap =
      LoadTextureFromImage(image); // Convert image to texture to display (VRAM)
  ...
  UnloadImage(
      image); // Unload cubesmap image from RAM, already uploaded to VRAM
