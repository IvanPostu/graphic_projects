...
int main() {
  ...
  Block blockTypes[BLOCK_MAX] = {
      ...
      GRASS_BLOCK, DIRT_BLOCK, STONE_BLOCK,  BEDROCK_BLOCK,   WATER_BLOCK,
      SAND_BLOCK,  LOG_BLOCK,  LEAVES_BLOCK, SAND_STONE_BLOCK};
  ...
  while (!WindowShouldClose()) {
    ...
    BeginDrawing();
    {
      ...
      BeginMode3D(cameraController.camera);
      {
        ...
        optional<Vector3> possibleOutlinedBlock = world.handleRaycastRequest(
            cameraController, raycastRequest, selectedBlock);
      ...
      {
        DrawLine(windowWidth / 2 - 10, windowHeight / 2, windowWidth / 2 + 10,
                 windowHeight / 2, BLACK);
        DrawLine(windowWidth / 2, windowHeight / 2 - 10, windowWidth / 2,
                 windowHeight / 2 + 10, BLACK);
      }
      {
...

        {
          ...
          std::string selectedBlockText =
              "- Selected block: " + getBlockName(selectedBlock.blockType);
          std::string fpsText = "- FPS: " + std::to_string((int)(1.0 / delta));
          std::string deltaText =
              "- Delta: " + std::to_string(delta * 1000.0f) + " ms";
          std::string cameraPosText =
              "- Camera: " +
              std::to_string((int)cameraController.camera.position.x) + ", " +
              std::to_string((int)cameraController.camera.position.y) + ", " +
              std::to_string((int)cameraController.camera.position.z);
          std::string chunksToGenerateText =
              "- Chunks to generate: " +
              std::to_string(world.chunksToGenerate.size());
          std::string chunksToRenderText =
              "- Chunks to render: " +
              std::to_string(world.chunksToRender.size());
          std::string totalChunksText =
              "- Total chunks: " + std::to_string(world.chunks.size());
