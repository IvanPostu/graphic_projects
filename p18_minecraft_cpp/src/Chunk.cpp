...
Block Chunk::getBlockInDirection(size_t x, size_t y, size_t z,
                                 tuple<int, int, int> dirTuple, World &world) {
  ...
  if (Chunk::inBounds(nx, ny, nz)) {
    ...
    tuple<int, int, int> neighborChunkIdx =
        std::make_tuple(neighborChunkX, neighborChunkY, neighborChunkZ);

    neighbor = world.getBlockAt(neighborChunkIdx, EUCMOD_SIMPLE(nx, CHUNK_SIZE),
...
void Chunk::generateModel(World &world, Atlas &atlas) {
  ...
  for (size_t i = 0; i < TOTAL_CHUNK_MESHES; i++) {
    // max vertices: 6 faces per block, 2 triangles per face, 3 vertices per
    // triangle, 3 floats per vertex
    newVertices[i] = (float *)malloc(CHUNK_SIZE * CHUNK_SIZE * CHUNK_SIZE * 6 *
                                     2 * 3 * 3 * sizeof(float));

    // max colors: 6 faces per block, 2 triangles per face, 3 vertices per
    // triangle, 4 floats per vertex (rgba)
    newColors[i] =
        (unsigned char *)malloc(CHUNK_SIZE * CHUNK_SIZE * CHUNK_SIZE * 6 * 2 *
                                3 * 4 * sizeof(unsigned char));

    // max texcoords: 6 faces per block, 2 triangles per face, 3 vertices per
    // triangle, 2 floats per vertex
    newTexcoords[i] = (float *)malloc(CHUNK_SIZE * CHUNK_SIZE * CHUNK_SIZE * 6 *
                                      2 * 3 * 2 * sizeof(float));
  }
    ...
    for (size_t y = 0; y < CHUNK_SIZE; y++) {
      for (size_t z = 0; z < CHUNK_SIZE; z++) {
        ...
        if (block.blockType == BlockType::WATER) {
          Block upNeighbor =
              this->getBlockInDirection(x, y, z, allDirTuples[Dir::Top], world);
        ...
        for (size_t i = 0; i < 6; i++) { // 6 faces per block
          ...
          if ((!neighbor.transparent // neight has non-see through faces
               || (block.blockType == BlockType::WATER &&
                   neighbor.blockType == BlockType::WATER) // water on water
               ) &&
              !(block.blockType == BlockType::WATER && dir == Dir::Top &&
                neighbor.blockType !=
                    BlockType::WATER) // avoid shortenZ weridness
          ) {
            continue;
          }
          ...
          for (size_t j = 0; j < 2; j++) {   // 2 triangles per face
            for (size_t k = 0; k < 3; k++) { // 3 vertices per triangle
              ...
              size_t texcoordTriangleOffset =
                  allTexcoordsTriangleOffsets[i][j][k];
              Vector2 cubeTexcoord = CUBE_TEXCOORDS[texcoordTriangleOffset];
              Vector2 texcoordScaled =
                  Vector2Scale(cubeTexcoord, 1.0f / TEXCOORDS_DIVISOR);
              Vector2 texcoordOffset = Vector2Scale(block.getTexcoords(dir),
                                                    1.0f / TEXCOORDS_DIVISOR);
              ...
              newVertices[cmi][newVertexCounts[cmi] * 3 + 2] =
                  pos.z + CUBE_VERTICES[allTriangleOffsets[i][j][k]].z *
                              (shortenZ ? 0.8f : 1.0f);
  ...
  if (!allBlank) {
    ...
    for (size_t i = 0; i < TOTAL_CHUNK_MESHES; i++) {
      ...
      newMeshes[i].vertices =
          (float *)malloc(newMeshes[i].vertexCount * 3 * sizeof(float));
      newMeshes[i].colors = (unsigned char *)malloc(newMeshes[i].vertexCount *
                                                    4 * sizeof(unsigned char));
      newMeshes[i].texcoords =
          (float *)malloc(newMeshes[i].vertexCount * 2 * sizeof(float));

      std::memcpy(newMeshes[i].vertices, newVertices[i],
                  newMeshes[i].vertexCount * 3 * sizeof(float));
      std::memcpy(newMeshes[i].colors, newColors[i],
                  newMeshes[i].vertexCount * 4 * sizeof(unsigned char));
      std::memcpy(newMeshes[i].texcoords, newTexcoords[i],
                  newMeshes[i].vertexCount * 2 * sizeof(float));

      UnloadMesh(this->oldMeshes[i]);
      UploadMesh(&newMeshes[i], false);

      this->models[i] = LoadModelFromMesh(newMeshes[i]);
      this->models[i].materials[0].maps[MATERIAL_MAP_DIFFUSE].texture =
          atlas.texture; // way to do this for all models?
...
Chunk::handleRayCollision(RayCollision rayCollision) const {
  ...
  for (size_t x = startX; x < endX; x++) {
    for (size_t y = startY; y < endY; y++) {
      for (size_t z = startZ; z < endZ; z++) {
        ...
        Vector3 spot = {(float)x, (float)y, (float)z};
...
void Chunk::placeBlockAt(tuple<size_t, size_t, size_t> blockIdx,
                         Vector3 rayNormal, Block block, World &world) {
  ...
  if (Chunk::inBounds(newBlockX, newBlockY, newBlockZ)) {
    ...
    world.dirtyNeighbors(this->position,
                         std::make_tuple(newBlockX, newBlockY, newBlockZ));
  } else {
    ...
    if (possibleChunk.has_value()) {
      ...
      world.dirtyNeighbors(
          newChunk.position,
          std::make_tuple(chunkBlockX, chunkBlockY, chunkBlockZ));
    }
  }
}