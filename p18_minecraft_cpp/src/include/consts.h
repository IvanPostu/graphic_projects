...
// #define BRANCHLESS_EUCMOD(a, b)  (((a) < 0) * ((((a) % (b)) + (b)) % (b)) +
// ((a) >= 0) * ((a) % (b)))

#define EUCMOD(a, b)                                                           \
  ((a) < 0 ? ((((a) % (b)) + (b)) % (b))                                       \
           : ((a) % (b)))                       // range of a is (-inf, inf)
...
#define RAND_CHANCE(chance)                                                    \
  (((float)rand() / (float)RAND_MAX) < (chance)) // returns bool

#define TUP_TO_STR(tup)                                                        \
  std::to_string(std::get<0>(tup)) + "," + std::to_string(std::get<1>(tup)) +  \
...
const int WORLD_SIZE =
    16; // approx. max world size vertically for chunk generation (in chunks)
