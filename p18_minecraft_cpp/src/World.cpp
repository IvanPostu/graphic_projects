...
World::World() {
  ...
  this->compareChunksByDistance =
      [this](const reference_wrapper<Chunk> &chunkRef1,
             const reference_wrapper<Chunk> &chunkRef2) -> bool {
...
optional<reference_wrapper<Chunk>>
World::getChunkAt(tuple<int, int, int> chunkPos) {
  // Does this copy??
...
void World::generateChunk(PerlinNoise &pn, Atlas &atlas) {
  ...
  pair<tuple<int, int, int>, float> chunkToGenerate =
      this->chunksToGenerate.back();
  ...
  for (size_t j = 0; j < 6; j++) {
    ...
    tuple<int, int, int> neighborTup =
        std::make_tuple(std::get<0>(chunkTup) + std::get<0>(dirTuple),
                        std::get<1>(chunkTup) + std::get<1>(dirTuple),
                        std::get<2>(chunkTup) + std::get<2>(dirTuple));

    optional<reference_wrapper<Chunk>> possibleChunk =
        this->getChunkAt(neighborTup);
    if (possibleChunk.has_value()) {
...
int World::getHeightAt(PerlinNoise &pn,
                       vector<pair<Vector2, float>> &lowerResolutions, int x,
                       int y) { // static
    ...
    if (lowerResolution.second < 0) {
      ...
      for (int z = maxHeight; z >= LOWEST_SURFACE_Z; z--) {
        ...
        double noise = PerlinNoise3DWithOctaves(
            pn, (double)lowerResolutionPos.x / NOISE_DIVISOR,
  ...
  auto linearInterpolation = [](float x1, float y1, float x2, float y2,
                                float x3) -> float {
    float rise = y2 - y1;
    float run = x2 - x1;
    if (run == 0) {
  ...
  float height1 =
      linearInterpolation(upscaleds[0].x, heights[0], upscaleds[1].x,
                          heights[1], (float)x); // (0,0) -> (1,0)
  float height2 =
      linearInterpolation(upscaleds[2].x, heights[2], upscaleds[3].x,
                          heights[3], (float)x); // (0,1) -> (1,1)
  float height3 =
      linearInterpolation(upscaleds[0].y, height1, upscaleds[2].y, height2,
                          (float)y); // (0-y, height1) -> (1-y, height2)

  int scaledHeight = (int)roundf(height3);
  return scaledHeight;
}
void World::createBlockPlaceRequestAt(Chunk &chunk, int x, int y, int z,
                                      Block block,
  ...
  tuple<int, int, int> neighborTup =
      std::make_tuple(std::get<0>(chunk.position) + chunkXDiff,
...
void World::createChunkData(PerlinNoise &pn, Chunk &chunk) {
  ...
  vector<pair<Vector2, float>> lowerResolutions =
      vector<pair<Vector2, float>>();
  ...
  for (size_t x = 0; x < CHUNK_SIZE; x++) {
    ...
    for (size_t y = 0; y < CHUNK_SIZE; y++) {
      ...
      int scaledHeight =
          World::getHeightAt(pn, lowerResolutions, worldX, worldY);
      scaledHeights[x][y] = scaledHeight;

      biomeFlatMap.push_back(
          Biome::biomeFromType(getBiomeFromHeight(scaledHeight)));
    }
  }
  ...
  */
  for (size_t x = 0; x < CHUNK_SIZE; x++) {
    ...
    for (size_t y = 0; y < CHUNK_SIZE; y++) {
      ...
      if (chunkZHeight >= 0 &&
          chunkZHeight <
              CHUNK_SIZE) { // only run for chunk that contains the scaledHeight

        int secondBlockDepth =
            RAND(biome.secondBlockLevelMin, biome.secondBlockLevelMax);
        ...
        for (int z = secondBlockStart; z < chunkZHeight; z++) {
          this->createBlockPlaceRequestAt(
              chunk, x, y, z, biome.secondBlock,
              vector<BlockType>({BlockType::STONE}));
        }
      }
    }
  }
  ...
  // -------------------------------------------------------------------------//
  /*
          TREE_CHANCE chance to place a tree at every scaledHeight above
     WATER_LEVEL Trunk height: [4, 5] Leaves radius: 5w, 4h block around top of
     tree
  */
  for (size_t x = 0; x < CHUNK_SIZE; x++) {
    // int worldX = worldChunkX + (int)x;
    for (size_t y = 0; y < CHUNK_SIZE; y++) {
      ...
      Biome &biome = biomeFlatMap[x * CHUNK_SIZE + y];
      if (biome.biomeType ==
          BiomeType::NEAR_WATER) { // don't place trees on water or on water's
                                   // edge
        continue;
      }
      ...
      if (chunkZHeight >= 0 &&
        ...
        if (tree) {
          ...
          for (int z = treeStart; z < treeEnd; z++) {
            this->createBlockPlaceRequestAt(
                chunk, x, y, z, LOG_BLOCK,
                vector<BlockType>({BlockType::AIR, BlockType::LEAVES}));
          }
          ...
          for (int leavesX = leavesStartX; leavesX <= leavesEndX; leavesX++) {
            for (int leavesY = leavesStartY; leavesY <= leavesEndY; leavesY++) {
              ...
              bool leavesXOnEdge =
                  leavesX == leavesStartX || leavesX == leavesEndX;
              bool leavesYOnEdge =
                  leavesY == leavesStartY || leavesY == leavesEndY;

              bool leavesXOnInsideOfEdge =
                  leavesX == leavesStartX + 1 || leavesX == leavesEndX - 1;
              bool leavesYOnInsideOfEdge =
                  leavesY == leavesStartY + 1 || leavesY == leavesEndY - 1;

              if (leavesXOnEdge && leavesYOnEdge) {
                continue;
              }

              for (int leavesZ = leavesStartZ; leavesZ <= leavesEndZ;
                   leavesZ++) {
                if (leavesX == (int)x && leavesY == (int)y &&
                    leavesZ < leavesEndZ) {
                  continue;
                } else if ((leavesXOnEdge || leavesYOnEdge) &&
                           leavesZ >= leavesEndZ - 1) {
                  continue;
                } else if ((leavesXOnInsideOfEdge && leavesYOnInsideOfEdge) &&
                           leavesZ == leavesEndZ) {
                  continue;
                }

                this->createBlockPlaceRequestAt(
                    chunk, leavesX, leavesY, leavesZ, LEAVES_BLOCK,
                    vector<BlockType>({BlockType::AIR}));
              }
            }
          }
        }
    ...
    for (size_t j = 0; j < this->blockPlaceRequests.size(); j++) {
      ...
      if (request.chunkPos == chunk.position) {
        Block blockToBeReplaced =
            chunk.getBlockAt(request.x, request.y, request.z);
...
void World::dirtyNeighbors(tuple<int, int, int> srcChunk,
  ...
  vector<tuple<int, int, int>>
      neighborChunks; // could inlcude out of bounds idxs
...
void World::sortKeysToRender() {
  ...
  std::sort(this->chunksToRender.begin(), this->chunksToRender.end(),
            this->compareChunksByDistance);
  this->shouldSortChunksToRender = false;
}
void World::sortChunksToGenerate() {
  ...
  // sorted: farthest to closest
  std::sort(this->chunksToGenerate.begin(), this->chunksToGenerate.end(),
            [this](const pair<tuple<int, int, int>, float> &a,
                   const pair<tuple<int, int, int>, float> &b) {
...
optional<Vector3>
World::handleRaycastRequest(const CameraController &cameraController,
  ...
  Ray ray = {cameraController.camera.position, cameraController.calcForward()};
  // TODO: use chunksToRender and break when distance is out of neighboring
  // chunks
      ...
      for (int z = cameraChunkZ - 1; z <= cameraChunkZ + 1; z++) {
        ...
        if (possibleChunk.has_value()) {
          ...
          for (size_t i = 0; i < RAYCAST_CHUNK_MODELS_COUNT; i++) {
            ...
            RayCollision qRayCollision = GetRayCollisionMesh(
                ray, chunk.models[cmi].meshes[0], qTransform);
  ...
  if (rayCollisions.size() > 0) {
    ...
    tuple<size_t, size_t, size_t> bestBlockTuple =
        closestChunkCollision->handleRayCollision(closestRayCollision);
    ...
    if (raycastRequest == RaycastRequest::DESTROY_BLOCK) {
      ...
      closestChunkCollision->placeBlockAt(
          bestBlockTuple, closestRayCollision.normal, selectedBlock, *this);
    } else { // center of outlined block
      Vector3 blockCenter =
          (Vector3){(float)std::get<0>(bestBlockTuple) + 0.5f,
                    (float)std::get<1>(bestBlockTuple) + 0.5f,
                    (float)std::get<2>(bestBlockTuple) + 0.5f};
      Vector3 bestBlockOutlinePos =
          Vector3Add(blockCenter, closestChunkCollision->getWorldPos());
