...
static void UpdateModelAnimationBones(Model *model, ModelAnimation *anim1,
                                      int frame1, ModelAnimation *anim2,
                                      int frame2, float blend,
                                      bool upperBodyBlend);
...
int main(void) {
  ...
  bool upperBodyBlend = true; // Toggle: true = upper/lower body blending, false
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    // When upperBodyBlend is ON: upper body = attack (1.0), lower body = walk
    // (0.0) When upperBodyBlend is OFF: uniform blend at 0.5 (50% walk, 50%
    // attack)
    float blendFactor = (upperBodyBlend ? 1.0f : 0.5f);
    UpdateModelAnimationBones(&model, &anim0, animCurrentFrame0, &anim1,
                              animCurrentFrame1, blendFactor, upperBodyBlend);
    ...
    DrawText(TextFormat("[SPACE] Toggle blending mode: %s",
                        upperBodyBlend ? "Upper/Lower Body Blending"
                                       : "Uniform Blending"),
...
static bool IsUpperBodyBone(const char *boneName) {
  ...
  if (TextIsEqual(boneName, "spine") || TextIsEqual(boneName, "spine1") ||
      TextIsEqual(boneName, "spine2") || TextIsEqual(boneName, "chest") ||
      TextIsEqual(boneName, "upperChest") || TextIsEqual(boneName, "neck") ||
      TextIsEqual(boneName, "head") || TextIsEqual(boneName, "shoulder") ||
      TextIsEqual(boneName, "shoulder_L") ||
      TextIsEqual(boneName, "shoulder_R") ||
      TextIsEqual(boneName, "upperArm") ||
      TextIsEqual(boneName, "upperArm_L") ||
      TextIsEqual(boneName, "upperArm_R") ||
      TextIsEqual(boneName, "lowerArm") ||
      TextIsEqual(boneName, "lowerArm_L") ||
      TextIsEqual(boneName, "lowerArm_R") || TextIsEqual(boneName, "hand") ||
      TextIsEqual(boneName, "hand_L") || TextIsEqual(boneName, "hand_R") ||
      TextIsEqual(boneName, "clavicle") ||
      TextIsEqual(boneName, "clavicle_L") ||
      TextIsEqual(boneName, "clavicle_R")) {
    return true;
  }
  ...
  if (strstr(boneName, "spine") != NULL || strstr(boneName, "chest") != NULL ||
      ...
      strstr(boneName, "hand") != NULL ||
      strstr(boneName, "clavicle") != NULL) {
    return true;
  }
...
static void UpdateModelAnimationBones(Model *model, ModelAnimation *anim0,
                                      int frame0, ModelAnimation *anim1,
                                      int frame1, float blend,
                                      bool upperBodyBlend) {
  // Validate inputs
  if ((anim0->boneCount != 0) && (anim0->keyframePoses != NULL) &&
    ...
    for (int boneIndex = 0; boneIndex < boneCount; boneIndex++) {
      ...
      // If upper body blending is enabled, use different blend factors for
      // upper vs lower body
      if (upperBodyBlend) {
        ...
        if (isUpperBody)
          ...
          boneBlendFactor = 1.0f - blend; // Lower body: blend towards anim0
                                          // (walk) - invert the blend
      }
      ...
      Transform blended = {0};
      blended.translation =
          Vector3Lerp(animTransform0->translation, animTransform1->translation,
                      boneBlendFactor);
      blended.rotation = QuaternionSlerp(
          animTransform0->rotation, animTransform1->rotation, boneBlendFactor);
      blended.scale = Vector3Lerp(animTransform0->scale, animTransform1->scale,
                                  boneBlendFactor);

      // Convert bind pose to matrix
      Matrix bindMatrix = MatrixMultiply(
          MatrixMultiply(MatrixScale(bindTransform->scale.x,
                                     bindTransform->scale.y,
                                     bindTransform->scale.z),
                         QuaternionToMatrix(bindTransform->rotation)),
          MatrixTranslate(bindTransform->translation.x,
                          bindTransform->translation.y,
                          bindTransform->translation.z));

      // Convert blended transform to matrix
      Matrix blendedMatrix = MatrixMultiply(
          MatrixMultiply(
              MatrixScale(blended.scale.x, blended.scale.y, blended.scale.z),
              QuaternionToMatrix(blended.rotation)),
          MatrixTranslate(blended.translation.x, blended.translation.y,
                          blended.translation.z));

      // Calculate final bone matrix (similar to UpdateModelAnimationBones)
      model->boneMatrices[boneIndex] =
          MatrixMultiply(MatrixInvert(bindMatrix), blendedMatrix);
    }
    ...
    for (int m = 0; m < model->meshCount; m++) {
      ...
      for (int vCounter = 0; vCounter < vertexValuesCount; vCounter += 3) {
        ...
        for (int j = 0; j < 4; j++, boneCounter++) {
          ...
          if (boneWeight == 0.0f)
            continue;
          animVertex =
              (Vector3){mesh.vertices[vCounter], mesh.vertices[vCounter + 1],
          ...
          // Normals processing
          // NOTE: We use meshes.baseNormals (default normal) to calculate
          // meshes.normals (animated normals)
          if ((mesh.normals != NULL) && (mesh.animNormals != NULL)) {
            animNormal =
                (Vector3){mesh.normals[vCounter], mesh.normals[vCounter + 1],
      ...
      if (bufferUpdateRequired) {
        ...
        rlUpdateVertexBuffer(mesh.vboId[SHADER_LOC_VERTEX_POSITION],
                             mesh.animVertices,
                             mesh.vertexCount * 3 * sizeof(float), 0);
        if (mesh.normals != NULL)
          rlUpdateVertexBuffer(mesh.vboId[SHADER_LOC_VERTEX_NORMAL],
                               mesh.animNormals,
                               mesh.vertexCount * 3 * sizeof(float), 0);
      }
    }
