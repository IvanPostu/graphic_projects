...
int main(void) {
  ...
  camera.projection =
      CAMERA_PERSPECTIVE; // Projection type (Standard 3D perspective)
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    // Find the current direction frame based on the camera position to the
    // billboard object
    float dir = (float)floor(
        ((Vector2Angle((Vector2){2.0f, 0.0f},
    ...
    // Draw billboard pointing straight up to the sky, rotated relative to the
    // camera and offset from the bottom
    DrawBillboardPro(
        camera, skillbot,
        (Rectangle){0.0f + (anim * 24.0f), 0.0f + (dir * 24.0f), 24.0f, 24.0f},
        Vector3Zero(), (Vector3){0.0f, 1.0f, 0.0f}, Vector2One(),
        (Vector2){0.5f, 0.0f}, 0, WHITE);
