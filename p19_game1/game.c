...
typedef enum State { STATE_MAIN_MENU, STATE_RUNNING, STATE_GAME_OVER } State;
...
int main(void) {
  ...
  Texture2D texture =
      LoadTexture("assets/test.png"); // Check README.md for how this works
  ...
  while (!WindowShouldClose()) {
    ...
    case STATE_MAIN_MENU: {
      draw_text_centered("Bullet Hell", (Vector2){SCREEN_WIDTH / 2.0f, 40.0f},
                         80);
      draw_text_centered("Press 'Enter' to start!",
                         (Vector2){SCREEN_WIDTH / 2.0f, 200.0f}, 20);
    ...
    case STATE_RUNNING: {
      ...
      // Shoot Proj
      {
        ...
        while (IsMouseButtonDown(MOUSE_LEFT_BUTTON) &&
          ...
          Vector2 dir = Vector2Normalize(
              Vector2Subtract(GetMousePosition(), gameState.playerPos));
      ...
      // Move the player
      {
        ...
        DrawCircle(gameState.playerPos.x, gameState.playerPos.y,
                   gameState.playerSize, GREEN);
      }
      ...
      // Update Projectiles
      {
        for (int projIdx = 0; projIdx < gameState.projCount; projIdx++) {
          ...
          const Rectangle screenRect = {
              .height = SCREEN_HEIGHT, .width = SCREEN_WIDTH, .x = 0, .y = 0};
          ...
          for (int enemyIdx = 0; enemyIdx < gameState.enemyCount; enemyIdx++) {
            ...
            if (CheckCollisionCircles(enemy->pos, enemy->size, proj->pos,
                                      proj->size)) {
              ...
              *enemy = gameState.enemies[--gameState.enemyCount];
              ...
              if (gameState.kills % 25 == 0) {
                gameState.shootDelay -= 0.01f; // 0.3
              }
      ...
      {
        // Spawn Enemies
        const float spawnFrequency = 1.0f / (gameState.gameTime * 0.1f);
        const float enemySize =
            8 - gameState.gameTime / 20.0f; // Get Smaller over time
        ...
        while (gameState.spawnTime >= spawnFrequency) {
          ...
          Enemy enemy = {
              .pos = {gameState.playerPos.x + dir.x * dist,
                      gameState.playerPos.y + dir.y * dist},
              .size = enemySize,
          };
        ...
        // Update Enemies
        {
          for (int enemyIdx = 0; enemyIdx < gameState.enemyCount; enemyIdx++) {
            ...
            Vector2 dir = Vector2Normalize(
                Vector2Subtract(gameState.playerPos, enemy->pos));
            ...
            if (CheckCollisionCircles(enemy->pos, enemy->size,
                                      gameState.playerPos,
    ...
    case STATE_GAME_OVER: {
      ...
      draw_text_centered("Press 'Enter' to restart!",
                         (Vector2){SCREEN_WIDTH / 2.0f, 200.0f}, 20);
